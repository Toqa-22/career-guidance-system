...
async function load() {
    ...
    if (regErr || instErr || courseErr) {
        document.getElementById('staffAllocBody').innerHTML =
            `<tr class="admin-empty-row"><td colspan="7" style="color:#b91c1c;">Could not load data: ${(regErr || instErr || courseErr).message} — try reloading the page.</td></tr>`;
    ...
    (regs || []).filter(r => r.staff_number).forEach(r => {
        ...
        if (!groups.has(key)) groups.set(key, { staff_name: r.staff_name, staff_number: r.staff_number, phone_number: r.phone_number || '', registrations: [] });
        const group = groups.get(key);
        // Different registrations for the same staff number can, in theory,
        // have been submitted with a different phone number each time —
        // the first non-empty one found is shown, same "one identity"
        // simplification already used for staff_name/staff_number above.
        if (!group.phone_number && r.phone_number) group.phone_number = r.phone_number;
...
async function fetchAllRegistrations() {
    ...
    while (true) {
        const { data, error } = await client
            .from('registrations')
            .select('id, course_id, staff_name, staff_number, phone_number, institution_name_snapshot, attended')
