...
if (!pending || !pending.username || !pending.role || !pending.sessionToken) {
...
async function attemptSetPassword() {
    ...
    try {
        // change_own_password now authenticates via the session token
        // (proof this really is pending.username, established at login) —
        // not a bare client-supplied username, which previously meant
        // ANYONE could have called this RPC for ANY account with no proof
        // at all.
        const { error } = await client.rpc('change_own_password', {
            p_session_token: pending.sessionToken,
            p_new_password: newPassword
        });
        if (error) throw error;

        // Password set — now actually complete the login the same way a
        // normal sign-in would: real session, routed by role. Reusing the
        // SAME session token here rather than requesting a new one — it's
        // already valid and change_own_password does not revoke the
        // token that was used to call it (only OTHER sessions for the
        // account), so it remains good to use as the real session.
        sessionStorage.removeItem(PENDING_KEY);
        sessionStorage.setItem(AUTH_STORAGE_KEY, JSON.stringify({ username: pending.username, role: pending.role, fullName: pending.fullName || null, sessionToken: pending.sessionToken, loginAt: Date.now() }));
