...
if (!courseId) {
    ...
    client.from('courses').select('name, theme_color, description, course_date, instructor_name, seats, unlimited_seats, links_closed').eq('id', courseId).maybeSingle().then(({ data, error }) => {
        ...
        logDescEl.innerHTML = logDescription ? `<span class="comment-pin-icon">📌</span> <span class="comment-shimmer-text">${logDescription}</span>` : 'Log your sessions for this activity below.';
