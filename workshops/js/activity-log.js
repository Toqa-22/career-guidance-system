...
// A single day still just reads as one date; a real range reads as "From — To".
...
document.getElementById('addEntryBtn').addEventListener('click', async () => {
    ...
    if (!title || !entryDateFrom || !entryDateTo) {
        note.textContent = 'Please fill in the title and date range.';
        note.style.color = '#b91c1c';
        return;
    }
    if (entryDateTo < entryDateFrom) {
        note.textContent = 'The "To" date can\'t be before the "From" date.';
        note.style.color = '#b91c1c';
        return;
    }
    ...
    try {
        ...
        // Same missing-department bug as the registration form's first
        // entry (js/workshops.js) — dept is chosen right above and applied
        // to institution_name_snapshot, but was never saved onto the entry
        // row itself.
        const { error } = await client.from('activity_log_entries').insert({
            registration_id: currentRegistrationId,
            title, entry_date_from: entryDateFrom, entry_date_to: entryDateTo,
            department: dept
        });
