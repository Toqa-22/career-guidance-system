...
async function loadCourses() {
    const { data, error } = await client.from('courses').select('id, name, course_date, course_end_date, activity_type');
        ...
        chartInstances.coverage = new Chart(document.getElementById('chartCoverage'), {
            ...
            data: {
                ...
                datasets: [
                    ...
                    { label: 'Attendance', data: rows.map(r => r.attended), backgroundColor: BAR_LINE_PALETTE[0], borderRadius: 5 }
                ]
            },
            options: {
                responsive: true, maintainAspectRatio: false,
...
// ============================================================================
// Same label set as Hall Reservation's own "Type of Reservation" — but this
// is a completely separate field/column (courses.activity_type), used only
// to color-code each activity's pill on this calendar. No default color
// is picked arbitrarily "no type set" falls back to a neutral gray in both
// the pill and the legend below.
...
const ACTIVITY_TYPE_DEFAULT_COLOR = '#94A3B8';

function renderActivityCalLegend() {
    const box = document.getElementById('activityCalLegend');
    if (!box) return;
    // Every activity now requires a Type of Activity at creation (Create
    // Activity's field is no longer optional), so there's no "not set"
    // case left to show a fallback entry for here.
    box.innerHTML = Object.entries(ACTIVITY_TYPE_COLORS).map(([type, color]) =>
        `<span class="activity-cal-legend-item"><span class="activity-cal-legend-swatch" style="background:${color};"></span>${type}</span>`
    ).join('');
}
...
function renderActivityCalGrid() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        // "None" activities are deliberately excluded from the calendar
        // entirely — not shown even as a gray/uncategorized pill, since
        // that's specifically what "None" means here.
        const dayCourses = coursesCached.filter(c => c.course_date === key && c.activity_type !== 'None');
        const pills = dayCourses.slice(0, 2).map(c => {
            const color = ACTIVITY_TYPE_COLORS[c.activity_type] || ACTIVITY_TYPE_DEFAULT_COLOR;
            return `<span class="activity-cal-day-pill" title="${c.name}${c.activity_type ? ' — ' + c.activity_type : ''}" style="background:${color}22; color:${color}; border-color:${color}55;">${c.name}</span>`;
        }).join('');
...
function renderActivityCalDayTable(dateStr) {
    ...
    // Same exclusion as the calendar grid itself — a "None" activity
    // shouldn't appear in this day's detail list either.
    const items = coursesCached.filter(c => c.course_date === dateStr && c.activity_type !== 'None');
