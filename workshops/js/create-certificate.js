...
let currentCertId = null; // null = creating a new certificate; set = editing this specific one
...
function resetEditorState() {
    ...
    document.getElementById('certificateCommentInput').value = '';
...
async function loadCourses() {
    ...
    const preselectCourseId = params.get('course_id');

    if (certId) {
        // Editing one specific existing template — its own row's id is
        // the only thing that identifies it now that a course can have
        // more than one certificate.
        await loadCertificateById(Number(certId));
        if (currentCourseId) select.value = currentCourseId;
    } else if (preselectCourseId) {
        // Arrived here to create a NEW certificate for a specific course
        // (e.g. from a course's own page) — pre-select it, but never
        // auto-load any existing template for it, since there may be
        // several or none.
        select.value = preselectCourseId;
        selectCourseForNewCertificate(Number(preselectCourseId));
    }
}

// Selecting a course manually (or via a course_id preselect) always means
// "start a new certificate for this course" — never auto-loads an
// existing template, since a course can now have several. This is also
// what running the picker while already editing an existing certificate
// does: it starts a fresh one for the newly-picked course rather than
// reassigning the one being edited, which would be a surprising side
// effect of just browsing the dropdown.
function selectCourseForNewCertificate(courseId) {
    currentCourseId = courseId;
    currentCertId = null;
    resetEditorState();
    loadEvaluationOptionsForCourse(courseId, []);
    if (!courseId) return;
...
// Only evaluations belonging to the currently-selected course are ever
// listed — an evaluation for a different Activity would never have a
// matching evaluation_responses row for this certificate's participants,
// so offering it here would just be a confusing way to lock everyone out.
// A certificate can require several (or none) of them; each is a checkbox
// rather than a single dropdown option.
async function loadEvaluationOptionsForCourse(courseId, selectedEvaluationIds) {
    const box = document.getElementById('certRequiredEvaluationList');
    if (!box) return;
    const selectedSet = new Set((selectedEvaluationIds || []).map(Number));
    if (!courseId) {
        box.innerHTML = '<p class="section-hint" style="margin:0;">Choose a course above to see its evaluations.</p>';
        return;
    }
    const { data: evaluations } = await client.from('activity_evaluations').select('id, title').eq('course_id', courseId).order('created_at', { ascending: false });
    if (!evaluations || evaluations.length === 0) {
        box.innerHTML = '<p class="section-hint" style="margin:0;">This course has no evaluations/exams yet.</p>';
        return;
    }
    box.innerHTML = evaluations.map(ev => `
        <label style="display:flex; align-items:center; gap:8px; padding:6px 0; cursor:pointer; font-size:13.5px; font-weight:600; color:#334155;">
            <input type="checkbox" class="cert-required-eval-checkbox" value="${ev.id}" ${selectedSet.has(ev.id) ? 'checked' : ''} style="width:16px; height:16px; cursor:pointer;">
            ${ev.title}
        </label>
    `).join('');
}

function getSelectedRequiredEvaluationIds() {
    return Array.from(document.querySelectorAll('.cert-required-eval-checkbox:checked')).map(cb => Number(cb.value));
}

async function loadCertificateById(certId) {
    const myToken = ++loadRequestToken;
    resetEditorState();

    const { data: cert } = await client.from('certificates').select('*').eq('id', certId).maybeSingle();

    // Something newer superseded this request while it was in flight —
    // discard this result entirely rather than let it clobber whatever
    // the newer request already set.
    if (myToken !== loadRequestToken) return;

    if (!cert) {
        alert('That certificate template could not be found — it may have been deleted.');
        return;
    }

    currentCertId = cert.id;
    currentCourseId = cert.course_id;
    const course = coursesCache.find(c => c.id === cert.course_id);

    document.getElementById('certificateNameInput').value = cert.certificate_name || (course ? `Certificate for ${course.name}` : '');
    document.getElementById('certificateCommentInput').value = cert.comment || '';
    const { data: requiredRows } = await client.from('certificate_required_evaluations').select('evaluation_id').eq('certificate_id', cert.id);
    await loadEvaluationOptionsForCourse(cert.course_id, (requiredRows || []).map(r => r.evaluation_id));
...
document.getElementById('certCourseSelect').addEventListener('change', (e) => {
    selectCourseForNewCertificate(e.target.value ? Number(e.target.value) : null);
});
...
document.getElementById('saveCertificateBtn').addEventListener('click', async () => {
    ...
    const commentValue = document.getElementById('certificateCommentInput').value.trim();
    if (!commentValue) {
    ...
    try {
        ...
        // Preserve the existing public_slug when updating an already-saved
        // certificate, so its link never changes underneath anyone who
        // already has it.
        const publicSlug = currentCertId
            ? (await client.from('certificates').select('public_slug').eq('id', currentCertId).maybeSingle()).data?.public_slug || Math.random().toString(36).slice(2, 10)
        ...
        const requiredEvaluationIds = getSelectedRequiredEvaluationIds();
        ...
        // A course can now have several separate certificate templates, so
        // saving is keyed on the certificate's OWN id (updating one
        // specific existing row) rather than upserting by course_id
        // (which would only ever allow one certificate per course, and
        // could silently overwrite a different template for the same
        // course). No currentCertId means this is a brand new template —
        // insert it, then remember its new id in case Save is pressed
        // again in the same visit.
        if (currentCertId) {
            const { error: saveErr } = await client.from('certificates').update(payload).eq('id', currentCertId);
            if (saveErr) throw new Error('Save failed: ' + saveErr.message);
        } else {
            const { data: inserted, error: saveErr } = await client.from('certificates').insert(payload).select('id').single();
            if (saveErr) throw new Error('Save failed: ' + saveErr.message);
            currentCertId = inserted.id;
        }
