...
let hiddenActivityKeys = new Set(); // activities toggled out of the export for this session
...
// Re-renders from the already-loaded displayedActivities — used both after
// a fresh load and after toggling a row's hidden state, so toggling never
// needs to hit the database again.
function renderTable() {
    ...
    tbody.innerHTML = displayedActivities.map((a, i) => {
        ...
        const isHidden = hiddenActivityKeys.has(a.activity_key);
        return `
        <tr data-activity-key="${a.activity_key}" class="${isHidden ? 'pf-row-hidden' : ''}">
            ...
            <td>
                <button type="button" class="pf-btn pf-btn-small" data-action="save">Save</button>
                <button type="button" class="pf-btn pf-btn-small pf-btn-hide" data-action="toggle-hide">${isHidden ? 'Show' : 'Hide'}</button>
            </td>
        </tr>`;
    }).join('');
    ...
    tbody.querySelectorAll('[data-action="toggle-hide"]').forEach(btn => {
        btn.addEventListener('click', () => {
            const key = btn.closest('tr').dataset.activityKey;
            if (hiddenActivityKeys.has(key)) hiddenActivityKeys.delete(key);
            else hiddenActivityKeys.add(key);
            renderTable();
        });
    });
}
...
document.getElementById('pfExportBtn').addEventListener('click', () => {
    ...
    if (displayedActivities.every(a => hiddenActivityKeys.has(a.activity_key))) {
        alert('Every activity in this period is currently hidden — show at least one before exporting.');
        return;
    }
    ...
    let row = 8;
    const exportableActivities = displayedActivities.filter(a => !hiddenActivityKeys.has(a.activity_key));
    exportableActivities.forEach((a, i) => {
        const d = detailsByKey.get(a.activity_key) || {};
        const contactValue = d.contact_number != null ? d.contact_number : (a.suggestedContact || '');
        const traineeCountValue = d.trainee_count != null ? d.trainee_count : (a.suggestedParticipantCount || '');
        const rowVals = {
