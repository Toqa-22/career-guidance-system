        ...
        async function bookHallIfRequested(activityName) {
            ...
            for (const entry of entries) {
                const clash = relevantHallEntries.find(r =>
                    ...
                    // entry's own value (from <input type="time">) is
                    // "HH:MM" — comparing the raw strings breaks exactly at
                    // the back-to-back boundary ("11:00" < "11:00:00" is
                    // TRUE in JS string comparison). Slicing both to
                    // "HH:MM" fixes it.
