...
// this page is ever moved. Built relative to this page's own URL (../ goes
// from admin/ up to workshops/) rather than string-replacing the pathname —
// a literal substring replace silently returns the ORIGINAL string
// unchanged if it doesn't match exactly (e.g. a different deployed path
// structure), which would point the copied link at this same admin page.
const requestLinkUrl = new URL('../department-hall-request.html', window.location.href).toString();
document.getElementById('copyLinkBtn').addEventListener('click', async () => {
    try {
        await navigator.clipboard.writeText(requestLinkUrl);
        alert('Link copied!');
    } catch {
        alert('Could not copy automatically — please try again or copy it from the Open Link tab.');
    }
});
