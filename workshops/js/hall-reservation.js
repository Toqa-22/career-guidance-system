...
document.getElementById('hallWeeklyRepeatToggle').addEventListener('change', (e) => {
    const isWeekly = e.target.checked;
    document.getElementById('hallWeeklyRepeatBox').classList.toggle('hidden-element', !isWeekly);
    document.getElementById('hallWeeklyHint').style.display = isWeekly ? 'block' : 'none';
    document.getElementById('hallDatesContainer').classList.toggle('hidden-element', isWeekly);
    document.getElementById('hallAddDateBtn').classList.toggle('hidden-element', isWeekly);
});

// Builds the flat (date, start, end) list from the weekly-repeat pattern —
// same shape collectDateEntries() below produces from the manual rows, so
// everything downstream (conflict checking, insertion) doesn't need to know
// which mode was used.
function collectWeeklyRepeatEntries() {
    const startDate = document.getElementById('hallWeeklyStartDate').value;
    const start_time = document.getElementById('hallWeeklyStartTime').value;
    const end_time = document.getElementById('hallWeeklyEndTime').value;
    const weekCount = parseInt(document.getElementById('hallWeeklyCount').value, 10) || 0;
...
function setDefaultFormDates() {
    ...
    document.getElementById('hallWeeklyRepeatToggle').checked = false;
    document.getElementById('hallWeeklyRepeatBox').classList.add('hidden-element');
    document.getElementById('hallWeeklyHint').style.display = 'none';
    document.getElementById('hallDatesContainer').classList.remove('hidden-element');
    document.getElementById('hallAddDateBtn').classList.remove('hidden-element');
    document.getElementById('hallWeeklyStartDate').value = todayStr;
    document.getElementById('hallWeeklyStartTime').value = '';
    document.getElementById('hallWeeklyEndTime').value = '';
    document.getElementById('hallWeeklyCount').value = 4;
}
...
document.getElementById('hallReservationForm').addEventListener('submit', async (e) => {
    ...
    const isWeeklyRepeat = document.getElementById('hallWeeklyRepeatToggle').checked;
    let entries;

    if (isWeeklyRepeat) {
        const startDate = document.getElementById('hallWeeklyStartDate').value;
        const weeklyStart = document.getElementById('hallWeeklyStartTime').value;
        const weeklyEnd = document.getElementById('hallWeeklyEndTime').value;
        const weekCount = parseInt(document.getElementById('hallWeeklyCount').value, 10) || 0;
        if (!startDate || !weeklyStart || !weeklyEnd) {
            setFormNote('Please fill in the first date and both times for the weekly repeat.');
            return;
        }
        if (weeklyStart >= weeklyEnd) {
            setFormNote('End time must be after start time.');
            return;
        }
        if (weekCount < 1) {
            setFormNote('Please enter at least 1 week.');
            return;
        }
        entries = collectWeeklyRepeatEntries();
    } else {
