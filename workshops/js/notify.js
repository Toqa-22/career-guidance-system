(function () {
    ...
    function confirmCard(message, options) {
        ...
        return new Promise((resolve) => {
            ...
            overlay.querySelector('.form-toast-title').textContent = message;
            overlay.querySelector('.confirm-cancel').textContent = options.cancelLabel || 'Cancel';
            overlay.querySelector('.confirm-ok').textContent = options.okLabel || 'Yes';
    ...
    // which shows the same native "page says" dialog as confirm(). `fields`
    // is an array of { name, label, value, type, placeholder }. Resolves
    // with an object keyed by field name, or null if cancelled.
    // A centered modal card — unlike the plain toast notifications (which
    // stay small, in the corner, non-blocking), this one is deliberately
    // front-and-center: filling in a reason (or confirming something) is a
    // real pause in the task, not a passing notification.
        ...
        return new Promise((resolve) => {
            ...
            const fieldsHtml = fields.map((f, i) => `
                ...
                <input type="${f.type || 'text'}" class="form-toast-input" data-field="${f.name}"
                       ...
                       placeholder="${f.placeholder || ''}" autocomplete="${f.autocomplete || 'off'}" ${i === 0 ? 'autofocus' : ''}>
            `).join('');
