...
// ============================================================================
// reshapeArabicForPdf(text)
// jsPDF renders text left-to-right and does not perform Arabic contextual
// letter-shaping on its own -- even with a proper Arabic font (see
// amiri-font.js) loaded, Arabic letters render disconnected/in the wrong
// visual order without this step. This function:
//   1) Splits the string into runs of Arabic vs. non-Arabic characters, so
//      mixed text (an activity name with both Arabic and English/numbers)
//      keeps each part readable in its own script.
//   2) Reshapes each Arabic run into its correct Presentation-Forms glyphs
//      via ArabicReshaper.convertArabic() (initial/medial/final/isolated
//      forms -- what a proper Arabic rendering engine does automatically,
//      which jsPDF does not).
//   3) Reverses each Arabic run's characters, and reverses the overall run
//      order, so the text displays right-to-left as jsPDF draws it
//      left-to-right. This is a practical approximation of the Unicode
//      BiDi algorithm for short strings like names/titles -- not a full
//      BiDi implementation, but handles the common real-world cases here
//      (pure Arabic, pure Latin, or a straightforward mix of the two).
// Call this on any Arabic-containing string immediately before passing it
// to doc.text() or into an autoTable() head/body/foot array.
// ============================================================================
function reshapeArabicForPdf(text) {
    if (!text) return text;
    text = String(text);

    var isArabicChar = function (ch) {
    ...
    var runs = [];
    var current = '';
    var currentIsArabic = null;
    for (var i = 0; i < text.length; i++) {
        var ch = text[i];
        var chIsArabic = isArabicChar(ch);
    ...
    var hasArabic = false;
    for (var j = 0; j < runs.length; j++) { if (runs[j].arabic) hasArabic = true; }
    if (!hasArabic) return text;

    var processedRuns = runs.map(function (r) {
        if (r.arabic) {
            var shaped = ArabicReshaper.convertArabic(r.text);
