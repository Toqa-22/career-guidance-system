...
document.getElementById('reqWeeklyRepeatToggle').addEventListener('change', (e) => {
    const isWeekly = e.target.checked;
    document.getElementById('reqWeeklyRepeatBox').classList.toggle('hidden-element', !isWeekly);
    document.getElementById('reqWeeklyHint').style.display = isWeekly ? 'block' : 'none';
    document.getElementById('reqDatesContainer').classList.toggle('hidden-element', isWeekly);
    document.getElementById('reqAddDateBtn').classList.toggle('hidden-element', isWeekly);
});

// Same shape collectDateEntries() below produces from the manual rows, so
// everything downstream doesn't need to know which mode was used.
function collectWeeklyRepeatEntries() {
    const startDate = document.getElementById('reqWeeklyStartDate').value;
    const start_time = document.getElementById('reqWeeklyStartTime').value;
    const end_time = document.getElementById('reqWeeklyEndTime').value;
    const weekCount = parseInt(document.getElementById('reqWeeklyCount').value, 10) || 0;
...
function setDefaultFormDates() {
    ...
    document.getElementById('reqWeeklyRepeatToggle').checked = false;
    document.getElementById('reqWeeklyRepeatBox').classList.add('hidden-element');
    document.getElementById('reqWeeklyHint').style.display = 'none';
    document.getElementById('reqDatesContainer').classList.remove('hidden-element');
    document.getElementById('reqAddDateBtn').classList.remove('hidden-element');
    document.getElementById('reqWeeklyStartDate').value = todayStr;
    document.getElementById('reqWeeklyStartTime').value = '';
    document.getElementById('reqWeeklyEndTime').value = '';
    document.getElementById('reqWeeklyCount').value = 4;
}
...
document.getElementById('hallRequestForm').addEventListener('submit', async (e) => {
    ...
    const isWeeklyRepeat = document.getElementById('reqWeeklyRepeatToggle').checked;
    let entries;

    if (isWeeklyRepeat) {
        const startDate = document.getElementById('reqWeeklyStartDate').value;
        const weeklyStart = document.getElementById('reqWeeklyStartTime').value;
        const weeklyEnd = document.getElementById('reqWeeklyEndTime').value;
        const weekCount = parseInt(document.getElementById('reqWeeklyCount').value, 10) || 0;
