...
async function fetchAllStudents(){
  ...
  // (commonly 1000) -- an unbounded .select("*") on a table that has
  // grown past that silently returns only the first page, with no error
  // at all. Paginating in batches here is what guarantees every row comes
  // back regardless of how large the table gets over time -- this was
  // very likely why some trainees (ones outside whatever the first page
  // happened to cover) never appeared in any report.
  ...
  while (true) {
    ...
    if (data.length < PAGE_SIZE) break; // last page was partial -- nothing more to fetch
    from += PAGE_SIZE;
  }
...
async function fetchWaitlistStudents(){
  // Same pagination reasoning as fetchAllStudents() above -- an unbounded
  // query silently truncates at Supabase's default row limit as the table
  // grows, with no error to signal it happened.
  const PAGE_SIZE = 1000;
  let allRows = [];
  let from = 0;
