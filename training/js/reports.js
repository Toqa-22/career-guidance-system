...
function getFilteredReportStudents(){
  ...
  const list = reportsState.allStudents.filter(s => {
    ...
    // EVERY report unconditionally, even when browsing "جميع الفترات" (no
 ...
 */
// Just the logo + hospital name + subtitle -- this is what repeats on
// every printed page (see buildReportPageHTML's page-wrapper-table thead).
// The report-specific title/date below are NOT part of this, since they
// should only appear once, at the top of page 1 -- not repeated alongside
// the letterhead on every subsequent page too.
function minimalReportHeaderHTML(){
  return `
...
function reportHeaderHTML(title){
  ...
  return `
  <div class="report-header report-header-title-block">
...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  /* The border/spacing that separates the whole letterhead from the
     summary bar below belongs on the title block specifically -- it's the
     natural end of "header" content once the minimal logo header (which
     now repeats separately on every page) and this title block (shown
     once, page 1 only) sit right next to each other. */
  ...
  .report-header h1{ margin:0; font-size:24px; font-weight:800; color:#0F6CBD; }
  .report-header h2{ margin:4px 0 0; font-size:15.5px; font-weight:700; color:#4B5D71; }
  .report-header .r-title{ margin-top:10px; font-size:17px; font-weight:800; color:#1B2A3A; }
  .report-header .r-date{ margin-top:6px; font-size:13px; color:#8A97A6; }

  .summary-bar{
    display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;
    background:#F5F7FA; border-radius:12px; padding:12px 18px; margin-bottom:6px;
    font-size:14.5px; font-weight:700; color:#31465C;
  }
  .summary-bar span.count{ color:#0A8F6A; }
  .unique-note{
    margin:0 4px 20px; font-size:12px; color:#8A97A6; font-weight:600;
  }
  ...
  thead th{
    background:#0F6CBD; color:#fff; font-size:13.5px; font-weight:800;
    padding:10px 7px; text-align:center;
    word-break: break-word;
  }
  tbody td{
    padding:9px 7px; font-size:13px; text-align:center;
    border-bottom:1px solid #E3E8EE;
    word-break: break-word;
  }
  tbody tr:nth-child(even){ background:#FAFCFE; }
  tbody tr.total-row td{
    background:#E9F1FC; color:#0F6CBD; font-weight:800; font-size:13.5px;
    border-top:2px solid #0F6CBD;
  }

  /* This outer table's own <thead> is what makes the logo/title header
     repeat on every printed page -- browsers repeat a table's thead at
     the top of each page it spans, per the CSS Paged Media spec, the same
     mechanism already used just above for each report's own column
     headers repeating. Wrapping the WHOLE page in one table and putting
     the letterhead in ITS thead applies that same repetition to the
     letterhead itself. */
  .page-wrapper-table{ width:100%; border-collapse:collapse; }
  .page-wrapper-table > thead{ display: table-header-group; }
  .page-wrapper-table > tbody{ display: table-row-group; }
  ...
  @media (max-width: 640px){
    ...
    thead th, tbody td{ font-size:12px; padding:7px 5px; }
  }
</style>
</head>
<body>
...
const KASHF3_COLUMN_WIDTHS = [7, 60, 13, 19, 34, 34, 42, 22, 30, 19, 19, 15, 13, 16]; // A إلى N بالترتيب — عريضة للنصوص الطويلة بسطر واحد؛ B (اسم المتدرب) وE-F (المؤسسة التعليمية) وG (التخصص) موسّعة أكثر بطلب صريح؛ العمودان H وN أضيق عمداً لأن عنوانيهما مقسَّمان على سطرين
...
async function exportKashf3Report(){
  ...
  try {
    ...
    // ws.columns لم يُهيَّأ من قبل في أي مكان آخر بهذه الورقة — الاعتماد على
    // .forEach() لتعديل عناصر موجودة مسبقاً كان يمر فعلياً على مصفوفة فارغة
    // ولا يُغيّر شيئاً إطلاقاً بصمت (بلا أي خطأ). التعيين المباشر أدناه هو ما
    // يُنشئ تعريفات الأعمدة فعلياً بالعرض المطلوب.
    ws.columns = KASHF3_COLUMN_WIDTHS.map(width => ({ width }));
