...
function groupDuplicateStudents(students){
  const map = new Map();
  students.forEach(s => {
    // Same identity key as js/reports.js's studentUniqueKey() and
    // js/home.js's countUniquePeople() -- phone alone when present (the
    // more reliable, harder-to-vary field), name only as a fallback for the
    // rare record with no phone at all. Previously this required BOTH name
    // AND phone to match exactly, so the same person's name being entered
    // even slightly differently across their different department
    // enrollments (spacing, a typo, capitalization beyond what .trim()
    // catches) split them into extra "duplicate" groups here despite
    // sharing the same phone number -- inflating this page's count above
    // what Reports and Home already agree on.
    const phone = (s.phone || "").trim();
    const key = phone || `name:${(s.student_name || "").trim()}`;
