...
// A student enrolled in more than one department has one row PER
// department (see js/supabase.js's insertStudentsWithPeriods) — counting
// rows directly for a "how many trainees" figure counts that same person
// once per department they're in, not once as a person. Matches the same
// phone-first, name-fallback identity key js/reports.js already uses for
// its own totals, so both pages agree on what "one trainee" means.
function countUniquePeople(rows) {
    const seen = new Set();
    rows.forEach(r => {
        const phone = (r.phone || '').trim();
        seen.add(phone || `name:${(r.student_name || '').trim()}`);
    });
    return seen.size;
}
...
async function loadHome() {
    ...
    const { data: allStudents, error: studentsErr } = await supabaseClient
        .from('students')
        .select('phone, student_name, department, training_start, training_end, training_type, gender, registration_date, is_waitlist, academic_stage');
    ...
    renderKpiTile(0, countUniquePeople(confirmed));
    renderKpiTile(1, countUniquePeople(inTrainingNow));
    renderKpiTile(2, countUniquePeople(waitlisted));
